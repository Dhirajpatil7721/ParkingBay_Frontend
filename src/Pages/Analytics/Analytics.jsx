...
// // ── Vehicle config (matches ActiveBookings / History) ─────────────────────────
// const VEHICLE_MAP = {
...
//               style={{
//                 flex: 1, padding: '8px 0', fontSize: 14, fontWeight: 600, borderRadius: 12,
//                 border: `1px solid ${selectedYear === yr ? '#1a3c5e' : '#e5e7eb'}`,
//                 background: selectedYear === yr ? '#1a3c5e' : '#fff',
//                 color: selectedYear === yr ? '#fff' : '#6b7280',
...
//                 style={{
//                   padding: '10px 0', fontSize: 13, fontWeight: 600, borderRadius: 12,
//                   border: `1px solid ${isSelected ? '#1a3c5e' : '#e5e7eb'}`,
//                   background: isSelected ? '#1a3c5e' : '#fff',
...
//       <p style={{ fontSize: 13, marginBottom: 24, color: '#6b7280', lineHeight: 1.6 }}>
...
//           Close
//         </button>
//         <button onClick={onSelectMonth}
//           style={{ flex: 1, padding: '10px 0', background: '#1a3c5e', color: '#fff', border: 'none', borderRadius: 12, fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>
//           Select Month
//         </button>
//       </div>
//     </DialogContent>
...
//     boxShadow: '0 1px 3px rgba(0,0,0,0.06)', transition: 'transform 0.25s, box-shadow 0.25s',
...
//       </div>
//       {badge && (
//         <span style={{ fontSize: 11, fontWeight: 600, padding: '4px 12px', borderRadius: 999, background: '#e8eef4', color: '#1a3c5e' }}>
//           {badge}
//         </span>
//       )}
//     </div>
...
//     ? ((totalDaysWithBookings / analyticsData.daysInMonth) * 100).toFixed(1) : '0.0';
...
//         d.bookingCount > 20 ? '#10b981' : d.bookingCount > 15 ? '#1a3c5e' : d.bookingCount > 10 ? '#3b82f6' : '#ef4444'
//       ) || [],
...
//     scales: {
//       y: { beginAtZero: true, grid: { color: '#f1f5f9' }, ticks: { stepSize: 5, color: '#9ca3af' } },
...
//           </div>
//           <p style={{ fontSize: 13, color: '#9ca3af', fontWeight: 500 }}>Loading analytics…</p>
//         </div>
//       </div>
//     );
//   }
...
//               <button
//                 onClick={() => { const [y, m] = selectedMonthYear.split('-').map(Number); fetchAnalyticsData(y, m); }}
//                 style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 36, height: 36, borderRadius: 10, border: '1px solid #e5e7eb', background: '#fff', color: '#6b7280', cursor: 'pointer' }}
//                 title="Refresh"
//               >
...
//             <ChartCard icon={ShowChartIcon} title="Daily Bookings Trend"
//               badge={`Avg: ${analyticsData?.Avrage?.toFixed(1) || 0}/day`} delay="0.45s">
...
//             <ChartCard icon={CarIcon} title="Vehicle Type Distribution"
//               badge={`Total: ${analyticsData?.BookingCount || 0}`} delay="0.5s">
//               <div className="vehicle-dist-inner">
//                 <div style={{ flex: 1, minWidth: 220, height: 240 }}>
//                   <Pie data={vehicleChartData} options={pieOptions} />
//                 </div>
//                 <div style={{ flex: 1, minWidth: 220 }}>
//                   {vehicleTypes.map((item, index) => {
//                     const Icon = item.icon;
//                     const pct = analyticsData?.BookingCount
//                       ? ((item.value / analyticsData.BookingCount) * 100).toFixed(1) : '0';
//                     return (
...
//               <ChartCard icon={CreditCardIcon} title="Payment Methods"
//                 badge={`${paymentMethods.reduce((s, p) => s + p.value, 0)} txns`} delay="0.55s">
...
//             <ChartCard icon={AssessmentIcon} title="Monthly Summary"
//               badge={`${analyticsData?.monthName || ''} ${analyticsData?.year || ''}`} delay="0.55s">
...
//             <ChartCard icon={TrophyIcon} title="Top Performing Days"
//               badge={`Peak: Day ${peakDay?.day || 0} • ${peakDay?.bookingCount || 0} bookings`} delay="0.6s">
...
//                         animation: 'fadeSlideUp 0.4s ease both', animationDelay: `${index * 60}ms`,
//                       }}
//                         onMouseEnter={(e) => (e.currentTarget.style.boxShadow = '0 4px 12px rgba(0,0,0,0.08)')}
//                         onMouseLeave={(e) => (e.currentTarget.style.boxShadow = 'none')}
//                       >
//                         <div style={{ width: 28, height: 28, borderRadius: '50%', margin: '0 auto 10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: '#fff', background: colors[index] }}>
//                           #{index + 1}
//                         </div>
...
//       {/* ── Modals ── */}
//       <MonthPickerModal open={showMonthPicker} onClose={() => setShowMonthPicker(false)}
//         selectedMonthYear={selectedMonthYear} onSelect={setSelectedMonthYear} />

//       <NoDataModal open={showNoDataModal} onClose={() => setShowNoDataModal(false)}
//         onSelectMonth={() => { setShowNoDataModal(false); setShowMonthPicker(true); }} />
