...
//           icon: deleting ? <div style={{ width: 14, height: 14, border: '2px solid #dc2626', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} /> : <Delete sx={{ fontSize: 15 }} />,
...
//   const handleEditOpen = (member) => { setEditTarget(member); setForm({ name: member.name, mobile: member.mobile.toString(), password: '' }); setErrors({}); setShowPass(false); };

//   const handleEditSave = async () => {
//     const e = validate(form);
//     if (Object.keys(e).length) { setErrors(e); return; }
//     const result = await dispatch(updateWorker({ workerId: editTarget._id, workerData: { name: form.name.trim(), mobile: parseInt(form.mobile.replace(/\s/g, '')), ...(form.password.trim() && { password: form.password }) } }));
...
//           </div>
//           <p style={{ fontSize: 13, color: '#9ca3af', fontWeight: 500 }}>Loading staff…</p>
//         </div>
//       </div>
//     );
//   }
