...
//             <div>
//               <h1 style={{ fontSize: 'clamp(1.4rem, 3vw, 1.9rem)', fontWeight: 800, color: '#1a3c5e', margin: 0, letterSpacing: '-0.025em' }} onClick={openDeleteModal}>Booking History</h1>
...
//             </div>
//           </div> */}
// <div className="flex flex-wrap items-center gap-3 bg-white rounded-2xl border border-gray-200 shadow-sm p-3">
//   {/* Search */}
//   <div className="relative flex-1 min-w-[160px]">
//     <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"><SearchIcon sx={{ fontSize: 18 }} /></span>
//     <input type="text" placeholder="Search vehicles, type, payment..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-9 pr-8 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 focus:border-[#1a3c5e] focus:ring-2 focus:ring-[#1a3c5e]/20 outline-none" />
//     {search && <button onClick={() => setSearch('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400"><ClearIcon sx={{ fontSize: 16 }} /></button>}
//   </div>

//   {/* Date range */}
//   <div className="flex items-center gap-2 bg-gray-50 p-1 rounded-xl border border-gray-200">
//     <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="px-2 py-1.5 text-sm rounded-lg bg-transparent outline-none" />
//     <span className="text-gray-400 text-xs">to</span>
//     <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="px-2 py-1.5 text-sm rounded-lg bg-transparent outline-none" />
//   </div>

//   {/* Buttons */}
//   <div className="flex items-center gap-2">
//     <button onClick={handleFilter} className="px-4 py-1.5 text-sm font-semibold rounded-xl bg-[#1a3c5e] text-white hover:bg-[#0f2a44]">Apply</button>
//     <button onClick={handleClear} className="px-4 py-1.5 text-sm font-semibold rounded-xl border border-gray-200 bg-white text-gray-500 hover:bg-gray-50">Clear</button>
//   </div>

//   {/* Right controls */}
//   <div className="flex items-center gap-3 ml-auto">
//     <span className="text-sm text-gray-500 whitespace-nowrap">{filteredData.length} total</span>
...
//       </Select>
//     </FormControl>
//     <Tooltip arrow title={viewMode === 'list' ? 'Card view' : 'List view'}>
//       <button onClick={() => setViewMode(viewMode === 'list' ? 'card' : 'list')} className="flex p-1.5 border border-gray-200 rounded-lg bg-white hover:bg-gray-50">
//         {viewMode === 'list' ? <GridIcon sx={{ fontSize: 18 }} /> : <ListIcon sx={{ fontSize: 18 }} />}
//       </button>
//     </Tooltip>
//   </div>
    ...
    totalPages > 0 ? (
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '12px', marginTop: '16px', padding: '12px 0 4px' }}>
        <div style={{ fontSize: '13px', color: '#6b7280' }}>
          Showing {filteredData.length === 0 ? 0 : startEntry}–{endEntry} of {filteredData.length} entries
        </div>
